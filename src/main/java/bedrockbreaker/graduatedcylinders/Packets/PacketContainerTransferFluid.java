	...
	public static class Handler implements IMessageHandler<PacketContainerTransferFluid, IMessage> {
		...
		public IMessage onMessage(PacketContainerTransferFluid message, MessageContext ctx) {
			...
			FMLCommonHandler.instance().getWorldThread(ctx.netHandler).addScheduledTask(() -> {
				...
				int transferAmount = FluidHelper.getTransferAmount(held, under);
				if (transferAmount == 0) return;

				IFluidHandlerItem heldFluidHandler = FluidUtil.getFluidHandler(held);
				IFluidHandlerItem underFluidHandler = FluidUtil.getFluidHandler(under);

				FluidStack fluidStack = heldFluidHandler.getTankProperties()[0].getContents();
				if (fluidStack == null) fluidStack = underFluidHandler.getTankProperties()[0].getContents();
				if (fluidStack == null) return;
				fluidStack = new FluidStack(fluidStack, Math.abs(transferAmount));

				if (FluidUtil.tryFluidTransfer(transferAmount < 0 ? underFluidHandler : heldFluidHandler, transferAmount < 0 ? heldFluidHandler : underFluidHandler, fluidStack, true) != null) player.world.playSound(null, player.getPosition(), transferAmount < 0 ? fluidStack.getFluid().getEmptySound(fluidStack) : fluidStack.getFluid().getFillSound(fluidStack), SoundCategory.PLAYERS, 1.0F, 1.0F);
